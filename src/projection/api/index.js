...
import { sourceEvent, useEvent } from "./useEvent.js";
import { useProjection } from "./useProjection.js";
import { sourceState, useState } from "./useState.js";
  ...
  (settings) => (name, projectionBehavior, getStateFlow) => {
    ...
    const finalProjectionBehavior = (...args) => {
      try {
        ...
        error.message =
          `Projection "${finalName}" execution error: ${error.message}`;
        throw error;
      }
    };
    ...
    const initialInputs = !settings.eventTypes &&
      stateFlows.map(({ getValue }) => getValue());

    const initialState = settings.initialState !== undefined
...
export const createProjectionSetupAPI = () => {
  ...
  const setupAPI = objectFrom(
    setupAPIRaw.map(([key, { func }]) => [key, func]),
  );

  const preventOutOfScopeUsage = chain(
    ...setupAPIRaw.map(
      ([key, { setup }]) => () => setup(throwUnexpectedScope(key)),
    ),
  );
