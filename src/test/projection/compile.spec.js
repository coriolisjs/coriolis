...
import { createReducedState } from '../../projection/reducedState'
import { createReducedStateChain } from '../../projection/reducedStateChain'

describe(`Projection compiler
  Converts a projection into a reducer and an initial state
`, () => {
  it(`Given a simple projection using events and an initial state
      When compiled
  ...
  it(`Given a two projections, one using the other
      When compiled
      Then we get a reducer and an initial state
    `, () => {
    ...
    const defaultGetStateFlow = (...args) => {
      ...
      return createReducedStateChain(createReducedState(reducer, initialState))
    }
